...
void init_cache() {          
    ...
    //define number of set
    S = 1;
    for(int i=0; i<s; i++){
        S = S*2;
    }
    //define number of block
    B = 1;
    for(int i=0; i<b; i++){
        B = B*2;
    }
...
void access_data(mem_addr_t addr) {      
    ...
    //updateline which represents line that will be changed 
    int updateLine = 0;

    //get Tag: move addr to right (s+b) to get a tag
    mem_addr_t Tag = addr >> (s+b);
    int sizeTag = 64-s-b;
    //get Set: move addr to left (tag) and move to right (64-s) to get a set
    int Set = (addr << sizeTag) >> (64-s);

    //hit operation
    for(int j=0; j<E; j++){
	//if valid and tag match
        if((*(cache+Set)+j)->valid == 1 && (*(cache+Set)+j)->tag == Tag){
	    //get global track to track LRU
            (*(cache+Set)+j)->trk = add_trk;
	    //add track to update which line is recently used
            add_trk++;
            hit_cnt++;
            return;
        }
    ...
    miss_cnt++;
    for(int j=0; j<E; j++){
	//there is one more invalid status to put data in line 
        if((*(cache+Set)+j)->valid == 0){
	    //set valid data to 1 to avoid being searched
            (*(cache+Set)+j)->valid = 1;
            (*(cache+Set)+j)->tag = Tag;
            (*(cache+Set)+j)->trk = add_trk;
            add_trk++;
    ...
    //evict
    for(int j=0; j<E; j++){
	//compare values to indentify which line is least recently used
        if((*(cache+Set)+j)->trk < (*(cache+Set)+updateLine)->trk){
            updateLine = j;
        }
    }
    evict_cnt++;
    (*(cache+Set)+updateLine)->valid = 1;
    (*(cache+Set)+updateLine)->tag = Tag;
    (*(cache+Set)+updateLine)->trk = add_trk;
    add_trk++;
}
